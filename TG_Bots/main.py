...
async def start_command(update: tg.Update, context: tgext.CallbackContext):
    ...
    await update.message.reply_text("Hi there! I`m only test bot.")

async def help_command(update: tg.Update, context: tgext.CallbackContext):
    print(f"{YELLOW_TEXT}User ({update.message.chat.id}: {update.message.from_user.username}) in {update.message.chat.type} ({update.message.chat.title}): \"{update.message.text}\"{FORMAT_CLEAR}")
    await update.message.reply_text("I`m bot only for testing some staffs. Now i can:\nCount a unique users that wrote messages (/count y/m/w/d/h/m/s).\nReply some messages (for example write 'f/F' or 'f.')")

async def m_num_command(update: tg.Update, context: tgext.CallbackContext):
    print(f"{YELLOW_TEXT}User ({update.message.chat.id}: {update.message.from_user.username}) in {update.message.chat.type} ({update.message.chat.title}): \"{update.message.text}\"{FORMAT_CLEAR}")
    await update.message.reply_text(f"There are {message_num} messages since {datetime.datetime.fromtimestamp(int(time.time() - data_cache.get_sec_diff()))}")

async def m_all_num_command(update: tg.Update, context: tgext.CallbackContext):
    print(f"{YELLOW_TEXT}User ({update.message.chat.id}: {update.message.from_user.username}) in {update.message.chat.type} ({update.message.chat.title}): \"{update.message.text}\"{FORMAT_CLEAR}")
    await update.message.reply_text(f"There are {update.message.message_id} messages for all time")
...
async def message_handler(update: tg.Update, context: tgext.CallbackContext):
    '''Check all messages'''
    chat_type = update.message.chat.type
    global message_num
    message_num += 1
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Sticker.ALL, message_sticker_handler),group=1)
    app.add_handler(MessageHandler(filters.PHOTO, message_photo_handler), group=2)
    app.add_handler(MessageHandler(filters.ANIMATION, message_animation_handler), group=2)
    app.add_handler(MessageHandler(filters.ALL, message_handler), group=5)
